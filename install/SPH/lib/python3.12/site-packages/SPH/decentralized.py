...
class Diff_SPH(Node): 
    def __init__(self): 
        ...
        self.get_logger().info(f'Nó iniciado para Robô ID: {self.id}')
    ...
    def control_loop(self): 
        ...
        # A partícula deste robô específico
        p = self.particles[self.id]
        
        # Atualiza a física da partícula com a posição real do odômetro
        p.x = self.robot_x[self.id] 
        p.y = self.robot_y[self.id] 
        p.ext_force = self.pot.force(p.x, p.y) 
        ...
        # Conversão de coordenadas (SPH -> Differential Drive)
        vx_ref = p.vel[0] 
        vy_ref = p.vel[1]  
        
        rtheta = self.robot_theta[self.id]
        v_ref = vx_ref * math.cos(rtheta) + vy_ref * math.sin(rtheta) 
        w_ref = (1.0 / self.d) * (-vx_ref * math.sin(rtheta) + vy_ref * math.cos(rtheta)) 

        # --- HISTÓRICO MANTIDO --- 
        self.hist_x_rob[self.id].append(p.x) 
        self.hist_y_rob[self.id].append(p.y) 
        self.hist_rho[self.id].append(p.rho) 
        self.hist_P[self.id].append(p.P) 
        self.hist_vel[self.id].append(p.vel_norm())

        # Publicação (usando self.id em vez de i)
        twist = Twist() 
        twist.linear.x = float(np.clip(v_ref, -0.5, 0.5)) 
        twist.angular.z = float(np.clip(w_ref, -1.0, 1.0)) 
        self.pubs[self.id].publish(twist)                                 
